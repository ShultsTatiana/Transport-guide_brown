...
bool doubleCompare(double x1, double x2, double epsilon) {
...
ostream& operator<<(ostream& out, const Stream::Bus& lhs) {
	out << lhs.name << ": ";
...
namespace Stream{
	void test1() {
		...
		stringstream input;
		input << inStr;
		auto groundRequest = Stream::ReadRequests(input);
		TransportGuide guide;
		guide.readRequests(groundRequest);
		auto requests = Stream::ReadRequests(input);
...
void poolOfTEsts() {
	TestRunner tr;
	RUN_TEST(tr, Stream::test1);
	RUN_TEST(tr, Stream::test2);
	RUN_TEST(tr, Stream::test3);
